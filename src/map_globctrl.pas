...
Uses
  ...
  main, map_datastreams, map_geometry, map_wkt, Dialogs,
  IniFiles;
...
Begin
  ...
  Location.Lat := frmmain.CDS.FieldByName('stlat').AsFloat;
  Location.Lon := frmmain.CDS.FieldByName('stlon').AsFloat;
  Center := Location;
  ReCenter;
end;
...
Begin
 Marker.Lat := frmmain.CDS.FieldByName('stlat').AsFloat;
 Marker.Lon := frmmain.CDS.FieldByName('stlon').AsFloat;
 Refresh;
end;
   ...
   if cursor=crHandPoint then begin
     for i:=0 to High(MapDataset) do begin
       if (X>=MapDataset[i].x-Pointer_Radius) and (X<=MapDataset[i].x+Pointer_Radius) and
          ...
          if frmmain.CDS.Locate('absnum', MapDataset[i].ID, [])=true then begin
  ...
  //     showmessage('9.22');
     ...
     pen.Style := psSolid;
      ...
      if SCount>0 then begin
       cur_id:=frmmain.CDS.FieldByName('absnum').AsInteger;
       ...
       for k:=0 to high(MapDataset)-1 do begin
         ...
         frmmain.CDS.Locate('absnum', cur_id, []);
