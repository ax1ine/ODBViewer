...
uses main, profile_flag, profile_plot_all;
...
Qt1.Database:=frmmain.DB1;
Qt1.Transaction:=TRt;

Qt2:=TSQLQuery.Create(self);
Qt2.Database:=frmmain.DB1;
Qt2.Transaction:=TRt;
...
ss:=0;
 ...
 try
  ...
  (* loop over tables *)
  for ks:=0 to frmmain.ListBox1.Items.Count-1 do begin
    Par:=frmmain.ListBox1.Items.Strings[ks];
  ...
  Count_st:=0; cur_l:=-9;
    while not Qt.eof do begin
      Lev_m   :=Qt.FieldByName('LEVEL_').AsVariant;
      Val_    :=Qt.FieldByName('VALUE_').AsFloat;
      QF      :=Qt.FieldByName('FLAG_').AsInteger;
 ...
 //     showmessage(par+#9+floattostr(lev)+#9+floattostr(val));
  ...
  {    if cur_l=lev then memo1.Lines.Add(frmosmain.listbox1Items.Strings[k]+
      ...
      while not CDS.Eof do begin
        if (CDS.FieldByName('LEV_M').AsFloat=Lev_m) then begin
            CDS.edit;
 ...
 (* settings for the grid *)
  DBGrid1.Columns[0].Title.Caption:='Level, m';
...
ChangeID(frmmain.CDS.FieldByName('ID').Value);
end;
